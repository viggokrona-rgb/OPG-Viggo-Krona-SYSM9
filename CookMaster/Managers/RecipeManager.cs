...
namespace CookMaster.Services
{
    ...
    public sealed class RecipeManager : ObservableObject, IRecipeManager
    {
        ...
        private RecipeManager()
        {
            ...
            if (Recipes.Count == 0)
            {
                Recipes.Add(new Recipe {CreatedBy = new User {Username = "user" },Title = "Spaghetti Bolognese", Ingredients = "Spaghetti, minced beef, tomato sauce, onion, garlic, olive oil, salt, pepper", Instructions = "Cook pasta. Brown beef with onion and garlic. Add sauce and simmer. Combine with pasta.", DateCreated = DateTime.Now.AddDays(-10), Category = "Dinner", Time = "30" });
                Recipes.Add(new Recipe { CreatedBy = new User { Username = "user" }, Title = "Chicken Curry", Ingredients = "Chicken, curry paste, coconut milk, onion, bell pepper, rice", Instructions = "Sauté onion and pepper, add chicken, then curry paste and coconut milk. Simmer. Serve with rice.", DateCreated = DateTime.Now.AddDays(-5), Category = "Dinner", Time = "40" });
